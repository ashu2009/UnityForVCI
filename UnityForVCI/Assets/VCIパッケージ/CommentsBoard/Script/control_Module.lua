...
local commentsBoardControlModule = require("CommentsBoardControl_Module")
local COMMENTS_BOARD_DEFINE = require("CommentsBoardControlDefine_Module")
local SYNC_TIME = 1
...
local function initialize()
    ...
    for i = 1, commentsBoardControlModule.getSubitemCount() do
        syncCls.commentsState[i] = COMMENTS_BOARD_DEFINE.NO_STATE
        ...
        if DEFINE.SYNC_ITEM.IsMine then
            ...
            while #syncCls.stackTasks ~= 0 do
                ...
                elseif task.controlNum == DEFINE.CONTROLKEY_GRAB then
                    syncCls.stackTasks, taskCount = taskDelete(syncCls.isNoTaskSkip, syncCls.stackTasks, taskCount)
                    -- 落下中のみ
                    if (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.FALL_STATE) and
                        (task.toWhat == COMMENTS_BOARD_DEFINE.GRAB_STATE) then
                        syncCls.commentsState[task.what] = task.toWhat
                    elseif (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.GRAB_STATE) and
                        (task.toWhat == COMMENTS_BOARD_DEFINE.FALL_STATE) then
                ...
                elseif task.controlNum == DEFINE.CONTROLKEY_USE then
                    syncCls.stackTasks, taskCount = taskDelete(syncCls.isNoTaskSkip, syncCls.stackTasks, taskCount)
                    -- 落下中、grab中のみ
                    if ((syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.FALL_STATE) or
                        (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.GRAB_STATE)) and
                        (task.toWhat == COMMENTS_BOARD_DEFINE.FIX_STATE) then
                        syncCls.commentsState[task.what] = task.toWhat
                    elseif (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.FIX_STATE) and
                        (task.toWhat == COMMENTS_BOARD_DEFINE.DELETE_STATE) then
                        syncCls.commentsState[task.what] = task.toWhat
                ...
                elseif task.controlNum == DEFINE.CONTROLKEY_DELETE then
                    ...
                    -- 固定中のみ
                    if ((syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.FALL_STATE) or
                        (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.FIX_STATE) or
                        (syncCls.commentsState[task.what] == COMMENTS_BOARD_DEFINE.GRAB_STATE)) then
                        syncCls.commentsState[task.what] = COMMENTS_BOARD_DEFINE.DELETE_STATE
                    end
                    allSyncMessage()
                    break
                end
...
function updateAll()
    -- コメント制御処理
    commentsBoardControlModule.commentControl(syncCls.ownerName, syncCls.commentsChar, syncCls.commentsState,
        syncCls.commentsTime)
    ...
    if DEFINE.SYNC_ITEM.IsMine then
        ...
        if #syncCls.comments > 0 then
            ...
            enable, syncCls.commentsChar, syncCls.commentsState, syncCls.commentsTime =
                commentsBoardControlModule.fallComment(syncCls.comments[1], syncCls.commentsChar, syncCls.commentsState,
                    syncCls.commentsTime)
            if enable then
        ...
        if time >= SYNC_TIME then
            syncTime = os.clock()
            syncCls.commentsTime = commentsBoardControlModule.addTime(syncCls.commentsState, syncCls.commentsTime, time)
            ---時間同期メッセージ(タスク追加)
...
function controlModule.onUse(use)
    commentsBoardControlModule.onUse(use, syncCls.commentsState)
end
...
function controlModule.onUngrab(target)
    commentsBoardControlModule.onUngrab(target, syncCls.commentsState)
end
